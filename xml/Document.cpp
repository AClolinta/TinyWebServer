...
Element Document::parse() {
    ...
    if (m_str[m_idx + 0] == '<' && m_str[m_idx + 1] == '?' &&
        m_str[m_idx + 2] == 'x' && m_str[m_idx + 3] == 'm' &&
        m_str[m_idx + 4] == 'l') {
    ...
    // parse xml element's
    if ((m_str[m_idx + 0] == '<') &&
        (isalpha(m_str[m_idx + 1]) || m_str[m_idx + 1] == '_')) {
        m_idx++;
        ...
        while (m_str[m_idx] != '\0') {
            ...
            if (m_str[m_idx + 0] == '/') {
                ...
                } else if (m_str[m_idx + 0] == '<' && m_str[m_idx + 1] == '!' &&
                           m_str[m_idx + 2] == '-' && m_str[m_idx + 3] == '-') {
                    // parse xml comment
                    if (!parse_comment()) {
                        throw std::logic_error("xml comment is error");
...
string Document::parse_element_name() {
    ...
    if (isalpha(m_str[m_idx]) || (m_str[m_idx] == '_')) {
        ...
        m_idx++;
        while ((m_str[m_idx] != '\0') &&
               (isalnum(m_str[m_idx]) || (m_str[m_idx] == '_') ||
...
string Document::parse_element_attr_key() {
    ...
    if (isalpha(m_str[m_idx]) || (m_str[m_idx] == '_')) {
        out += m_str[m_idx];
        m_idx++;
        while (isalnum(m_str[m_idx]) || (m_str[m_idx] == '_') ||
               (m_str[m_idx] == '-') || (m_str[m_idx] == ':')) {
            out += m_str[m_idx];
            m_idx++;
        }
